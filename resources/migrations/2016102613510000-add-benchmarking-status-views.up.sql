...
COUNT(task_id) filter (where event.event_id IS NOT NULL)                                                    AS n_completed,
...
round((COUNT(task_id) filter (where event.event_id IS NOT NULL) / COUNT(task_id)::float)::numeric, 4) * 100 AS percent_completed,
