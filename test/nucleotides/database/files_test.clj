...
(deftest test-filename->species-name
  (let [file (io/file "tmp/input_data/controlled_vocabulary/source.yml")]
    (is (= "source" (files/filename->species-name file)))))
...
(deftest test-get-dataset-map
  ...
  (testing "on the data directory"
    (let [dir     "tmp/input_data/inputs/data"
          inputs  (files/get-dataset-map dir)]
      (is (contains? inputs "amycolatopsis_sulphurea_dsm_46092")))))
